...
// ================= Advertisement Mode =================
// mode=active  : Worker 在任务到来/完成时推送 STATUS
// mode=passive : Worker 按固定周期推送 STATUS（周期用 probePeriod）
// mode=probe   : 保留旧逻辑：Out 周期 PROBE，Worker 回 STATUS（可选）
enum class AdvMode { Active, Passive, Probe };

static AdvMode ParseAdvMode(std::string s)
{
  std::transform(s.begin(), s.end(), s.begin(),
                 [](unsigned char c){ return (unsigned char)std::tolower(c); });

  if (s == "active")  return AdvMode::Active;
  if (s == "passive") return AdvMode::Passive;
  if (s == "probe" || s == "legacy") return AdvMode::Probe;

  NS_LOG_UNCOND("[WARN] unknown --mode=" << s << ", fallback to active");
  return AdvMode::Active;
}
...
private:
  AdvMode m_advMode{AdvMode::Active};
  double  m_advPeriod{1.0};
  EventId m_advEv;
  struct Job {
    uint64_t id{0};
    uint64_t N{0};
  ...
  virtual void StartApplication() override
  {
    ...
    m_advMode   = ParseAdvMode(g_modeStr);
    m_advPeriod = g_probePeriod;

    // passive：启动周期通告（先发一次，避免一开始表太久不校准）
    if (m_advMode == AdvMode::Passive) {
      m_advEv = Simulator::Schedule(Seconds(0.05), &WorkerApp::PeriodicAdvertise, this);
    }
  ...
  virtual void StopApplication() override
  {
    ...
    if (m_advEv.IsRunning()) Simulator::Cancel(m_advEv);
  }
  ...
  {
    if (m_advMode != AdvMode::Passive) return;
    SendStatus("PERIODIC", 0, 0);
    m_advEv = Simulator::Schedule(Seconds(m_advPeriod), &WorkerApp::PeriodicAdvertise, this);
  }
  ...
  void SendStatus(const std::string &reason, uint64_t doneId, uint32_t freed)
  {
    ...
    bool quietLikeProbe = (reason == "PROBE" || reason == "PERIODIC");

    if (!quietLikeProbe) {
      NS_LOG_UNCOND("["<<m_name<<"] " << msg);
    } else {
      bool bad = (used > m_baseline) || (free > m_baseline);
      if (!g_quietProbe || (bad && ShouldPrintAnom(m_name + ":" + reason + "_BAD"))) {
        NS_LOG_UNCOND("["<<m_name<<"] " << msg);
      }
    }
...
private:
  virtual void StartApplication() override
  {
    ...
    AdvMode m = ParseAdvMode(g_modeStr);
    if (m == AdvMode::Probe) {
  ...
  void OnStatusFromWorker(Ptr<Socket> sock)
  {
    ...
    // 只静默 PERIODIC/PROBE（不影响 CSV 和转发）
    bool quietLikeProbe = (reason == "PROBE" || reason == "PERIODIC");
    if (!quietLikeProbe || !g_quietProbe) {
...
int main(int argc, char *argv[])
{
  ...
  // 兼容旧参数：mode 不再影响机制
  cmd.AddValue("mode", "Advertisement mode: active | passive | probe(legacy)", g_modeStr);

// passive: 作为 Worker 周期上报周期；probe: 作为 Out->Worker PROBE 周期
  cmd.AddValue("probePeriod", "Adv/Probe period (sec): passive=worker adv, probe=out probe", g_probePeriod);
